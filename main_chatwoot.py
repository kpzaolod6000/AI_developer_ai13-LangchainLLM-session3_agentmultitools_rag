...
# Etiqueta que activa el bot (opcional)
...
async def chatwoot_webhook(request: Request):
    ...
    # Verificar que tenga la etiqueta del bot (si está configurada)
    if BOT_LABEL and BOT_LABEL not in labels:
        print(f"   ⏭️  Ignorado: no tiene etiqueta '{BOT_LABEL}'")
        return {"status": "ignored", "reason": f"Missing label '{BOT_LABEL}'"}
...
def read_root():
    ...
    return {
        ...
        "model": "GPT-4.1",
        "tools": ["buscar_datapath", "buscar_internet"],
...
if __name__ == "__main__":
    ...
    print(f"🔧 Tools: buscar_datapath, buscar_internet")
    print(f"💾 Historial: PostgreSQL")
    print(f"🏷️  Etiqueta bot: {BOT_LABEL or 'ninguna'}")
    print("=" * 60)
    print()
